...
-- ----------------------------
-- Procedure structure for insert_room
-- ----------------------------
DROP PROCEDURE IF EXISTS `insert_room`;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `insert_room`(in floor int,in room_number int)
begin
    declare i int default 1;
		declare j int default 1;
    while i <= floor do
			while j <= room_number do
				if j < 10 then
					insert into `t_room`(`building`,`number`,`bed_total`) values
					('1号楼',concat(i,'0',j),4);
				else
					insert into `t_room`(`building`,`number`,`bed_total`) values
					('1号楼',concat(i,j),4);
...
-- ----------------------------
-- Procedure structure for insert_room_in
-- ----------------------------
DROP PROCEDURE IF EXISTS `insert_room_in`;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `insert_room_in`(in student_count int,in bed_total int)
begin
	...
	while i < student_count do
		...
		#找到最小的未入住学生的id
		set sid = (select min(`s_id`) from `t_student` where `sex` = '男' and `s_id` not in (select distinct `s_id` from `t_room_in`));

		#找到未住满的房间的最小id
		set rid = (select min(`r_id`) from `t_room` where `building` = '1号楼' and `r_id` not in (select distinct `r_id` from `t_room_in` group by `r_id` having count(`s_id`) = 4 ));
 		
		#找到未住满的房间的最小床位号
		set bid = (bid + 1) % 4;
		
		insert into `t_room_in`(`r_id`,`s_id`,`bed_number`) values (rid,sid,bid + 1);
		set i = i + 1;
	end while;
end
;;
...
-- ----------------------------
-- Procedure structure for insert_student
-- ----------------------------
DROP PROCEDURE IF EXISTS `insert_student`;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `insert_student`(in cid int,in age int,in student_count int)
begin
	declare i int default 0;
	...
	while i < student_count do
		...
		if rand_num > 5 then
			insert into `t_student`(`id`,`name`,`sex`,`age`,`phone`,`c_id`) values
			(concat('2017',FLOOR(RAND()*89999+10000)),rand_girl_name(1),'女',age,
			 concat('13',FLOOR(RAND()*899999999+100000000)),cid);
		else
			insert into `t_student`(`id`,`name`,`sex`,`age`,`phone`,`c_id`) values
			(concat('2017',FLOOR(RAND()*89999+10000)),rand_boy_name(1),'男',age,
			 concat('13',FLOOR(RAND()*899999999+100000000)),cid);
		end if;
		set i = i + 1;
	end while;
...
-- ----------------------------
-- Procedure structure for show_girl_name
-- ----------------------------
DROP PROCEDURE IF EXISTS `show_girl_name`;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `show_girl_name`(in num int)
begin
    -- 记录循环次数
    declare i int default 0;
    while i < num do
			select(rand_girl_name(1));
	    set i = i + 1;
    end while;
end
;;
...
-- ----------------------------
-- Function structure for rand_boy_name
-- ----------------------------
DROP FUNCTION IF EXISTS `rand_boy_name`;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` FUNCTION `rand_boy_name`(n int) RETURNS varchar(16) CHARSET utf8mb4
begin
	-- 初始化一个33姓氏字符串，作为姓氏字符库
    declare family_str varchar (255) default '赵钱孙李周吴郑王冯陈蒋沈韩杨朱秦尤许何吕张黄魏姜潘范严陶苏马唐方傅';
    -- 初始化一个147名字字符串，作为名字字符库
    declare name_str varchar (255) default '仁立朴杉松林振柄柏桓森栋材标桥权积程茂光旭志昊明昌炎忠炳昶炯南昱昭映晟焱皓智朝景普煊煜照焕彰赫福烨阳旷燊熹曦耀显晋烈彧大山中方石民屹均垚岳臣岸岩府厚奎信家城邦国基域培凯尧硕广磊坚上元天君京军威冠禹乾钧琨琛琨殿瑞铭锐锋璋锦义钟鑫隆镕镜銮声鼎兆江沛泳泉津凌洋洲浩海港渤湖汇霆霄泽鸿瀚潮霖涛济达通逸逵';
    -- 记录当前是第几个
    declare i int default 0;
    -- 记录生成结果
    ...
    while i < n do
	    ...
	    -- 随机取姓氏
	    set full_name = concat(full_name, SUBSTR(family_str, floor(1+rand()*33), 1));
	    -- 随机取名字
	    set full_name = concat(full_name, SUBSTR(name_str, floor(1+rand()*147), 1));
	    -- 名字是否为双字
	    set rand_num = rand()*10;
	    set full_name = if(rand_num > 3, concat(full_name, SUBSTR(name_str, floor(1+rand()*165), 1)), full_name);
	    set i = i + 1;
    end while;
...
-- ----------------------------
-- Function structure for rand_girl_name
-- ----------------------------
DROP FUNCTION IF EXISTS `rand_girl_name`;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` FUNCTION `rand_girl_name`(n int) RETURNS varchar(16) CHARSET utf8mb4
begin
	-- 初始化一个33姓氏字符串，作为姓氏字符库
    declare family_str varchar (255) default '赵钱孙李周吴郑王冯陈蒋沈韩杨朱秦尤许何吕张黄魏姜潘范严陶苏马唐方傅';
    -- 初始化一个165名字字符串，作为名字字符库
    declare name_str varchar (255) default '秀娟英华慧巧美娜静淑惠珠翠雅芝玉萍红娥玲芬芳燕彩春菊兰凤洁梅琳素云莲真环雪荣爱妹霞香月莺媛艳瑞凡佳嘉琼勤珍贞莉桂娣叶璧璐娅琦晶妍茜秋珊莎锦黛青倩婷姣婉娴瑾颖露瑶怡婵雁蓓纨仪荷丹蓉眉君琴蕊薇菁梦岚苑筠柔竹霭凝晓欢霄枫芸菲寒欣滢伊亚宜可姬舒影荔枝思丽秀飘育馥琦晶妍茜秋珊莎锦黛青倩婷宁蓓纨苑婕馨瑗琰韵融园艺咏卿聪澜纯毓悦昭冰爽琬茗羽希';
    -- 记录当前是第几个
    declare i int default 0;
    -- 记录生成结果
    declare full_name varchar(16) default '';
    ...
    while i < n do
	    ...
	    set full_name = concat(full_name, SUBSTR(family_str, floor(1+rand()*33), 1));
	    -- 随机取名字
	    set full_name = concat(full_name, SUBSTR(name_str, floor(1+rand()*165), 1));
	    -- 名字是否为双字
	    set rand_num = rand()*10;
	    set full_name = if(rand_num > 3, concat(full_name, SUBSTR(name_str, floor(1+rand()*165), 1)), full_name);
	    set i = i + 1;
